...
export function ProjectGrid({ projects, categories }: ProjectGridProps) {
  ...
  const filteredProjects =
    selectedCategory === "All"
      ...
      : projects.filter(
          (project) =>
            asString(project.frontmatter?.category) === selectedCategory
        );
    ...
    <article className="group overflow-hidden rounded-lg bg-white shadow-md transition-all hover:shadow-xl dark:bg-gray-800">
      <Link href={`/projects/${slug}`}>
        ...
        <div className="p-6">
          ...
          {category && (
            <div className="mb-3">
              <span className="pill bg-primary/10 text-primary text-xs">
                {category}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center gap-4 border-t border-gray-200 pt-4 dark:border-gray-700">
            ...
            {githubUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-1 text-sm text-gray-600 transition-colors hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200"
                onClick={(e) => e.stopPropagation()}
              >
                <svg
                  className="h-4 w-4"
